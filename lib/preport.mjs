...
import {verifNum} from './tdext.mjs';
...
export class preport {
	...
	constructor(param) {
		...
		this.table = new ptable('#preport', {extraRowFunc: param.extraRowFunc, conditions: this.conditions});
	...
	condField(condition) {
		...
		lbspan.textContent = this.translate(condition.name) + ' : ';
		...
		ispan.addEventListener("keyup", ()=>{
			this.table[condition.name] = parseFloat(ispan.textContent);
